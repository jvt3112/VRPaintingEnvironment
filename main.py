...
def reachedPointStartLevel():
    ...
    point[0] *= 10 
    point[1] *= -10
    global flag, wait, collectionList, timer, text_2D_screen, changeit, threshold, centre, checkPointList, levelOn, level, chances
    if levelOn==1:
        if timer>timeOut:
            changeit = 0
            flag = 0
...
def changeLevel():
    ...
    if level==0:
        startPoint = np.array([-3.5,2.0])
        endPoint = np.array([-1.5,2.0])
        level = 1
        clearDrawing()
        level1()
    elif level==1:
